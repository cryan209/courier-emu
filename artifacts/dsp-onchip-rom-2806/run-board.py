"""Deliver the on-chip ROM read to the 25 MHz 2806 and capture one half.

Adapted from artifacts/dsp-status-03/run-board.py. The one change of substance:
that script asserted the 20.16 MHz unit's own timer-0 vector (8000:108F) and PCB
timer word. This board is a different unit, so the vector is read and recorded
rather than asserted, and what is still checked is that the watchdog puts back
whatever was there.

    python artifacts/dsp-onchip-rom-2806/run-board.py <half-dir> <port>
"""
...
try:
    query('AT'); identity = query('ATI7'); validate_identity(identity)
    ...
    print(f'timer-0 vector on this unit: {original[1]:04X}:{original[0]:04X}, T0CON {timer:04X}', flush=True)
    (out/'vector.json').write_text(json.dumps({'timer0_vector': list(original), 't0con': timer}))
    changed = 0
    for start in range(base, base+len(ram), 256):
        old = page(start); chunk = ram[start-base:start-base+256]
        for i in range(0, len(chunk), 2):
            if chunk[i:i+2] != old[i:i+2]:
                write(start+i, int.from_bytes(chunk[i:i+2], 'little')); changed += 1
    ...
    (out/'frame.txt').write_bytes(data); log.write(data); log.flush()
    print(repr(data[:200]), flush=True)
    result = parse_capture(data[data.index(b'CDRP1 START'):])
    (out/'hardware.json').write_text(json.dumps(result, indent=2))
    print('words captured:', result.get('word_count'), flush=True)
