...
try:
 query('AT');identity=query('ATI7');_,target=validate_identity(identity);(out/'identity.txt').write_bytes(identity)
 ...
 assert result['word_count']==9 and result['words'][0]==0x9209, result
 result['status']='instruction-probe-captured'
 ...
 after=query('ATGLK2=0000:FF00');(out/'pcb-after.txt').write_bytes(after)
 rows=re.findall(rb'^ *0000:([0-9A-Fa-f]{4})\s+((?:[0-9A-Fa-f]{2} ){15}[0-9A-Fa-f]{2})',after,re.M)
 pcb_after=b''.join(bytes.fromhex(v.decode()) for a,v in rows)
 assert struct.unpack_from('<H',pcb_after,0x36)[0]==timer, 'timer not restored'
