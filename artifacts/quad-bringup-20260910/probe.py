...
for position in (0, 1):
    link = QuadUsart()
    link.queue(b'22\x02')
    board = QuadBoard(identity=position << 4, usart=link)
    machine = CourierMachine(image, port_values={0x260: position << 4},
        quad_board=board, tick_ms=5, serial_input=b'AT\r', max_io_events=100,
        pc_watch={0x819e6:'chassis_rx', 0x80730:'copy_image',
                  0x93804:'dsp_load', 0x80260:'service_loop'})
    result = machine.run(8_000_000)
    board.flush()
    for channel in range(4):
        (out / f'position-{position}-channel-{channel}-flash.bin').write_bytes(board.flash[channel])
        (out / f'position-{position}-channel-{channel}-ram.bin').write_bytes(board.ram[channel])
    row = dict(flash_hashes=[sha256(b).hexdigest() for b in board.flash],
        ram_hashes=[sha256(b).hexdigest() for b in board.ram],window_changes=board.window_changes,position=position, status=result.status, ticks=machine.ticks,
