...
capture('run_marker',[0xBF80,0x9202])
for ndx in (0,1):
 w.extend([0x5E07,0xFFFB] if not ndx else [0x5D07,4])
 capture(f'ndx{ndx}.pmst',[0x0807])
 w.extend([0xAE7B,0x1234])
 for name,code in [('direct',[0x007B]),('short',[0xB055]),('long',[0xBF08,0x4321]),('indirect',[0xBF09,0x007B,0x8B89,0x0080]),('samm',[0xBF80,0x2468,0x8810])]:
  sentinels();w.extend(code)
  for reg,op in [('arcr',0x0819),('indx',0x0818),('ar0',0x0810)]:capture(f'ndx{ndx}.{name}.{reg}',[op])
 # Independent INDX sentinel tests all defined non-ARP modes.
 for mode in (0,2,4,8,10,12,14):
  w.extend([0xBF80,3,0x8818,0xBF09,0x1234,0x8B89,0x8B80|(mode<<3)])
  capture(f'ndx{ndx}.mar{mode:x}.ar1',[0x0811])
