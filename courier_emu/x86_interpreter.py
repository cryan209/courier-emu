...
_SEGMENT_PREFIX: dict[int, int] = {
    0x26: UC_X86_REG_ES, 0x2E: UC_X86_REG_CS, 0x36: UC_X86_REG_SS, 0x3E: UC_X86_REG_DS,
}
...
class Uc:
    ...
    def emu_start(self, begin: int, _until: int, timeout: int = 0, count: int = 0) -> None:
        ...
        code_hooks = self._code_hooks
        retired = 0
        while self.running and (not count or retired < count):
            start_ip = cpu_regs[UC_X86_REG_IP]
            physical = (cpu_regs[UC_X86_REG_CS] * 16 + start_ip) & address_mask
            for callback, user, begin, end in code_hooks:
                if end == 0 or begin <= physical <= end:
            ...
            if _IS_PREFIX[opcode]:
                while _IS_PREFIX[opcode]:
                    if opcode in _SEGMENT_PREFIX:
                        segment_override = cpu_regs[_SEGMENT_PREFIX[opcode]]
            ...
            elif 0xB0 <= opcode <= 0xB7: self._set_reg8(opcode - 0xB0, self._fetch(1))
