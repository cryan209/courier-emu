...
REGISTERS: tuple[AsicRegister, ...] = (
    ...
    AsicRegister(
        "monitor", (), 0x50,
        "A scaled copy of the received codec sample, DSP to ASIC, with no CPU "
        "port at all. The frame ISR takes the sample from DRR and writes "
        "`(sample * @1d) >> 14` here - 0x8193 `lamm @20`, then 0x8196 "
        ...
        "from the 2806's M/L trace. A leased ROM pair writes it 539,452 "
        "times in 150M instructions, once per frame.\n\n"
        "WHAT CONSUMES IT IS UNKNOWN. The name is the one the M/L chase left "
        "on it and is probably wrong: the call-progress speaker is very "
        "likely the codec's MON OUT, which per the AC01 datasheet "
        "(docs/ac01.txt) 'allows monitoring of analog input' - a purely "
        "analog path from IN+/IN- to the pin, with gains squelch, 0, -8 and "
        "-18 dB in register 4 DS05-DS04, defaulting to squelch. That is four "
        "states against an AT set with exactly four (M0 off, L1/L2/L3), and "
        "it is the right behaviour for a call-progress speaker: the line is "
        "heard directly, in analog, whatever the datapump is doing.\n\n"
        "An earlier version of this comment argued that the existence of "
        "this register was evidence against MON OUT - that a codec-internal "
        "monitor would need no sample copy. That is a false dichotomy. MON "
        "OUT never touches the DSP's sample stream, so this register is "
        "independent of it and says nothing about the speaker either way.\n\n"
        "Still unmeasured on both counts: the MON OUT wiring was never put "
        "on a meter (docs/board.md withdraws it as unsupported, while "
        "docs/board-parts.md still states it as fact), and register 4 reads "
        "0x09 - monitor squelched - under every M and L setting in the "
        "harness, which matches the 2806 board, where RAM 0x0693 gated both "
        "programmable paths off.\n\n"
