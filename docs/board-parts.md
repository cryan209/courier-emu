...
| `ADM707` | supervisory/reset. **Pin 7, the active-high `RESET`, goes to ASIC pin 111**, so the ASIC is in the reset tree and its latches become a live candidate for the unidentified `WDI` source - which is pin 8, next door. Its 1.6 s watchdog is what bounds every probe run ([ram-probe-delivery.md](ram-probe-delivery.md)). See [asic-pinout.md](asic-pinout.md) |
