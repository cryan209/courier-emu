...
| `DS1489AM`, `U18` | the **EIA-232 line receiver** - the 1489 half of the 1488/1489 pair, EIA in / TTL out, DTE-to-modem. ASIC pin 48 is on its `4Y` output (pin 11). Its other three outputs - pins 3, 6 and 8 - carry the rest of the DTE's `TXD`/`DTR`/`RTS`, and which CPU pin `TXD` reaches decides the serial-channel question. See [asic-pinout.md](asic-pinout.md) |
