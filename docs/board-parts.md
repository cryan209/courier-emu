...
| Atmel 8-pin | serial EEPROM, the NVRAM the settings cache comes from. On the 2806 it is the **CPU's**, not the ASIC's: `CS`/`SK` on CPU pins 52/57 (`P1.5`/`P1.2`) and `DI`+`DO` tied onto CPU 79 (`P2.7`). `ORG` floats, which is x16 - 256 words, matching `nvram.py`. See [asic-pinout.md](asic-pinout.md) |
