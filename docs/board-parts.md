...
| `74VHC573`, `74VHC32`, `74VHC04` | bus glue. The '573 is an **address demultiplex latch**; there is exactly **one** '573 and it latches the **high** byte, `AD8`-`AD15` into `A8`-`A15`, with a `74VHC32` beside it. Nothing else identified latches `A0`-`A7`, which is what puts that job on the ASIC - see [asic-pinout.md](asic-pinout.md) |
