...
| `PA28F400` | the **flash**. Intel 4 Mbit / 512 KiB, which is the 2806 dump exactly. Pinout confirmed against the Am29F400B 44-lead SO connection diagram - `A17`/`A7`/`A0` on pins 3/4/11 all match the board readings. `BYTE#` is tied high, so it runs in word mode and its `A`n is system `A`n+1 - pin 3 is system `A18`, pin 34 system `A17`. one high address pin comes from ASIC pin 73 - read as pin 3 (`A17`) and later as pin 34 (`A16`), see [asic-pinout.md](asic-pinout.md) |
