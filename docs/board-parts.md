...
| `H11B2` x2, `U14` and `U16` | **optocouplers** - the line-side isolation barrier, and the first named parts in the DAA section. Pin 5 of each, on the transistor side, goes to ASIC pins 19 and 22, so the ASIC **receives** both. Two isolated inputs from the line is the shape of ring detect plus loop-current sense; ring sense already has a port (`0x14` bit 1). See [asic-pinout.md](asic-pinout.md) |
