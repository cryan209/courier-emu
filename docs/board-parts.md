...
| `TLC320AC03` codec | its serial interface goes **direct to the DSP** - `DOUT`/`DIN`/`FS` (FN pins 11/10/12) on DSP 43/106/104, each one pin from its TDM counterpart. The ASIC's single line to it is **`MCLK`** (pin 14, from ASIC 112), so the ASIC generates both audio timebases. `MON OUT` (pin 1) is the part's call-progress monitor output and the leading candidate for the speaker. See [asic-pinout.md](asic-pinout.md) |
