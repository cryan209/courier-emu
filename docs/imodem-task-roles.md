...
| TID_MODEM / 12 | Not one of the C queue-wait loops | Starts at `a400:0008`, initializes DSP-related state, then continues the modem firmware path. Uses callbacks into the call-control/layer-4 code. | Modem supervisor task, confirmed name/startup. An AT exchange is now verified: `ATI2` answers `OK` and `ATI3` the banner over SIO0 ([the AT interface](imodem-at-interface.md)). |
