...
recovered `0x0649 = 5` event, removes dial tone, and starts DTMF on the active
C52. This advances the original supervisor to `OK`; a quiet or disconnected
...
the ASIC's confirmed 9.6 kHz line cadence. DTMF remains generated by the C52
line-output path and is carried in-band rather than synthesized as SIP events.
...
→ C52 audio → PCMU RTP. SIP responses are not yet mapped back into the complete
Courier carrier/result-code state machine.
